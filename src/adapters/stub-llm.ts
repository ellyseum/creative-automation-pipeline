...
import type {
  ...
  LLMMessage, LLMResponse, ToolDeclaration,
...
const CANNED_PLAN = JSON.stringify({
  ...
  products: [
    { productId: 'solar-flask', strategy: 'reuse', assetPath: 'assets/products/solar-flask/hero.jpg', assetSimilarity: 0.92, rationale: 'Stub: matched existing product hero', compositionNotes: 'Subject centered, overlay bottom-third' },
    { productId: 'dawn-brew', strategy: 'generate', generationDirection: 'Organic tea in a ceramic cup, warm morning light, minimalist', rationale: 'Stub: no matching asset found', compositionNotes: 'Subject centered, overlay bottom-third' },
  ],
});
...
const CANNED_BRAND_CHECK = JSON.stringify({
  onBrand: true, paletteUsage: 'adequate', toneMatch: 'strong',
  issues: [], suggestionsForRegeneration: [], severity: 'none',
});

const CANNED_ASSET_METADATA = JSON.stringify({
  description: 'Product photo on neutral background, warm tones',
  tags: ['product-shot', 'warm-palette'], mood: 'aspirational',
  subjects: ['product'], setting: 'studio',
